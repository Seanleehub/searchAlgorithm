...
int manhattan(int xi,int yi,int xj,int yj)
{
	return distance(xi,xj) + distance(yi,yj);
}
...
void ucs_path(struct map *map,	int x0, int y0, int x1, int y1)
{
	...
	/* Add start node*/	
	val = y0 * (map->width) + x0;
	prio = manhattan(x0, y0, x1, y1);
	pq_enqueue(pq, val, prio);
	map->grid[val].flags |= SQ_FLAG_ENQUEUED;
	while(pq_size(pq) > 0){		
		pq_dequeue(pq, &val, &prio);		
		if((map->grid[val].totalCost + manhattan(val%map->width,val/map->width,x1,y1)) < prio){
			continue;
		}	
			...
			if((SQ_FLAG_IMPASSABLE & (map->grid[nVal].flags)) == 0){
				if((SQ_FLAG_VISITED & (map->grid[nVal].flags)) == 0){
					if((SQ_FLAG_ENQUEUED & (map->grid[nVal].flags)) == 0){
						map->grid[nVal].totalCost = map->grid[val].totalCost + map->grid[nVal].cost;
						nPrio = map->grid[nVal].totalCost + manhattan(nVal%map->width,nVal/map->width,x1,y1);
						...
						nPrio = map->grid[val].totalCost + (map->grid[nVal].cost);
						if(nPrio < (map->grid[nVal].totalCost)){
							/* Update total cost */
							map->grid[nVal].totalCost = nPrio;
							nPrio += manhattan(nVal%map->width,nVal/map->width,x1,y1);							
							map->grid[nVal].prev = (map->grid) + val;
							if(!(pq_enqueue(pq, nVal, nPrio))){
								fprintf(stderr, "Min heap is full.\n");
								break;
